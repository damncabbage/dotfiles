...
"set encoding=utf-8 too...?

function! AddRubyDebug()
  let debug_method_name = "dbg!"

  let debug_method_code = []
  call add(debug_method_code, "require \"pathname\"")
  call add(debug_method_code, "module Kernel")
  call add(debug_method_code, "  def " . debug_method_name . "(val, name)")
  call add(debug_method_code, "    loc = caller_locations(1, 1).first")
  call add(debug_method_code, "    path = Pathname.new(loc.path).relative_path_from(Pathname.new(Dir.pwd)).to_s")
  call add(debug_method_code, "    puts \"[#{loc.base_label}, #{path}:#{loc.lineno}] [#{name}]: #{val.inspect}\"")
  call add(debug_method_code, "    val")
  call add(debug_method_code, "  end")
  call add(debug_method_code, "end")
...
nnoremap <leader>dd :call AddRubyDebug()<CR>
